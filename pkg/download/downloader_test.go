...
func TestCancelledQueuedIDPreservesItsPartArtifact(t *testing.T) {
	dir := t.TempDir()
	...
	go func() {
		done <- d.downloadPhotosFromPages(ctx, 2, 1, true, func(context.Context, int) ([]api.Photo, error) {
			return []api.Photo{
	...
	if !exists(queuedPart) {
		t.Fatal("artifact for an enqueued but unprocessed ID must not be swept")
	}
	if d.progress.Stats().Failed != 0 || len(d.progress.Stats().Failures) != 0 {
		t.Fatalf("cancelled worker recorded failures: %+v", d.progress.Stats().Failures)
	}
}
